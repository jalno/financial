	...
	<div class="col-xs-12">
		<form action="<?php echo userpanel\url('transactions/product/delete/'.$this->product->id); ?>" method="POST" role="form" class="form-horizontal">
			<div class="alert alert-block alert-warning fade in">
				...
				<p>
					<?php echo Translator::trans('transaction.product.delete.warning', ['product.id' => $this->product->id]); ?>
				</p>
				<p>
					...
					<button <?php if ($error = $this->getFormErrorsByInput('products')) {
					    echo 'disabled';
					} ?> type="submit" class="btn btn-yellow"><i class="fa fa-trash-o tip"></i> <?php echo Translator::trans('ticket.delete'); ?></button>
				</p>
			</div>
		</form>
	</div>
