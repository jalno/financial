...
<div class="row">
	<div class="col-xs-12">
		<div class="invoice">
			...
			<hr>
		<?php if ($this->hasdesc or (Transaction::paid == $this->transaction->status and $refundInfo)) { ?>
			<div class="row">
				<div class="col-md-12">
					<div class="box-note">
					<?php
...
?>
			...
			<?php
            if ($refundTransaction) {
                if (Transaction::expired == $this->transaction->status) {
                    ?>
				<div class="alert alert-info text-center"><?php echo t('packages.financial.refunded-expired-refund-transaction'); ?></div>
			<?php
                } elseif (Transaction::rejected == $this->transaction->status) {
                    ?>
			<div class="alert alert-warning"><?php echo t($refundInfo ? 'packages.financial.rejected-refund-transaction-reasoned' : 'packages.financial.rejected-refund-transaction', [
			    'reason' => $refundInfo,
			]); ?></div>
			<?php
                }
					...
					<table class="table table-striped table-hover">
						<thead>
							<tr>
								...
								<?php if (Transaction::paid == $this->transaction->status and !$this->transaction->isConfigured()) { ?><th></th>‍<?php } ?>
							</tr>
						</thead>
						<tbody>
						<?php
...
foreach ($this->transaction->products as $product) {
    ...
    ?>
							<tr>
								...
								<td class="hidden-480"><?php echo t('product.xnumber', ['number' => $product->number]); ?></td>
			...
			<div class="row">
                ...
                foreach ($this->pays as $pay) {
                    if ($hasButtons) {
                        ...
                        $this->setButtonActive('pay_accept', $this->canPayAccept and TransactionPay::pending == $pay->status);
                        $this->setButtonActive('pay_reject', $this->canPayReject and TransactionPay::pending == $pay->status);
                    }
                    if ($hastatus) {
