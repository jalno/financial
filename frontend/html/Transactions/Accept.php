...
<form class="transaction-accept-form" action="<?php echo userpanel\url("transactions/accept/{$this->transaction->id}"); ?>" method="POST" role="form">
	<div class="alert alert-block alert-success fade in">
		...
		<p class="text-warning">
			...
			<?php echo t('transaction.accept.warning.all_pays_will_accept', ['pending_pays_count' => $pendingPaysCount]); ?>
		</p>
		<?php } ?>
		<?php if ($remainPrice > 0) { ?>
		<p class="text-warning">
			<i class="fa fa-exclamation" aria-hidden="true"></i>
			<?php echo t('transaction.accept.warning.add_new_pay_for_remain_price', [
			    'price' => $remainPrice,
