...
use packages\financial\{Authorization, Transaction, Transaction_Pay, Transaction_Product};
...
class Merge extends Logs {
	...
	private function generateHTMLForTransaction(Transaction $transaction, bool $isMerged = false) {
		...
		if (isset($transaction->data["products"]) and $transaction->data["products"]) {
			$products = array_map(function($item) {
				return is_array($item) ? new Transaction_Product($item) : $item;
		...
		if (isset($transaction->data["pays"]) and $transaction->data["pays"]) {
			$pays = array_map(function($item) {
				return is_array($item) ? new Transaction_Pay($item) : $item;
	...
	private function getPayStatusText(Transaction_Pay $pay): string {
		switch ($pay->status) {
	...
	private function getPayStatusClass(Transaction_Pay $pay): string {
		switch ($pay->status) {
			case Transaction_Pay::rejected: return 'label label-danger';
			case Transaction_Pay::accepted: return 'label label-success';
			case Transaction_Pay::pending: return 'label label-warning';
			case Transaction_Pay::REIMBURSE: return 'label label-info';
			default: return "";
		}
	}
}
