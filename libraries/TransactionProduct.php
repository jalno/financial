...
use packages\financial\{Events\GateWays\InputNameException, TransactionsProductParam};
...
class TransactionProduct extends DBObject{
		...
		if(is_array($data)){
			if(isset($data['params'])){
				foreach($data['params'] as $name => $value){
					$this->tmparams[$name] = new TransactionsProductsParam(array(
						'name' => $name,
						'value' => $value
					));
				}
	...
	public function setParam($name, $value){
		...
		if(!$param){
			$param = new TransactionsProductsParam(array(
				'name' => $name,
				'value' => $value
			));
		}else{
